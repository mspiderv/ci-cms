...
class Carabiner {
    ...
    private function _combine($flag, $files, $filename) {
        ...
        foreach ($files as $file):
            ...
            if ((isset($file['minify']) && $file['minify'] == true) || (!isset($file['minify']) && $minify)):
                ...
                $r = ( is_url($file[$v]) ) ? $file[$v] : realpath($path . $file[$v]);
    ...
    private function _minify($flag, $file_ref) {
        ...
        $path = ($flag == 'css') ? $this->style_path : $this->script_path;
        $ref = ( is_url($file_ref) ) ? $file_ref : realpath($path . $file_ref);
        ...
        switch ($flag) {
            ...
            case 'css':
                ...
                $rel = ( is_url($file_ref) ) ? $file_ref : dirname($this->style_uri . $file_ref) . '/';
    ...
    private function _get_contents($ref) {
        ...
        if (is_url($ref) && ( ini_get('allow_url_fopen') == 0 || $this->force_curl )):
    ...
    private function _tag($flag, $ref, $cache = FALSE, $media = 'screen') {
        ...
        switch ($flag) {
            ...
            case 'css':
                ...
                $dir = ( is_url($ref) ) ? '' : ( ($cache) ? $this->cache_uri : $this->style_uri );
            ...
            case 'js':
                ...
                $dir = ( is_url($ref) ) ? '' : ( ($cache) ? $this->cache_uri : $this->script_uri );
