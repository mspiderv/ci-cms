...
class Eshop_categories extends CI_Driver {
    ...
    function get_categories_select_data($category_id = '', $unselectable = TRUE)
    {
        ...
        {
            if(is_array($category) && $this->CI->e_categories_model->item_exists(@$category['id']) && (!$this->CI->e_categories_model->item_exists($category_id) || $this->valid_parent_category_id($category['id'], $category_id)))
            {
                $select_data[$category['id']] = repeater('-&nbsp;', @$category['level']) . $this->CI->e_categories_model->$category['id']->_name;
            }
        }
    ...
    {
        if(!$this->CI->e_categories_model->item_exists($category_id) || !$this->CI->e_categories_model->item_exists($parent_category_id)) return FALSE;
    ...
    function get_categories_structure($parent_id = NULL, $level = 0)
    {
        ...
        if((int)$parent_id > 0 && !$this->CI->e_categories_model->item_exists($parent_id)) return array();
        
        $order_cat_col = $this->CI->e_categories_model->get_col('order_cat');
        
        if((int)$parent_id > 0)
        {
            $this->CI->e_categories_model->where($order_cat_col, '=', $parent_id);
        }
        else
        {
            $this->CI->e_categories_model->where($order_cat_col, '=', '');
        }
        
        foreach($this->CI->e_categories_model->get_ids() as $category_id)
        {
            $categories_structure[] = array(
                'id' => $category_id,
                'level' => $level
    ...
    function get_category_parent($category_id = '')
    {
        ...
        {
            return $this->CI->e_categories_model->get_item_data($category_id, $this->CI->e_categories_model->get_col('order_cat'));
        }
        else
        {
            return NULL;
